///On my honor, I pledge that I have neither received nor provided improper assistance in the completion of this assignment. Signed: [minchan Park] Student Number: [21800270] Section: [2]
...
pNode more(pList p, int z) {
	...
	for(pNode node=begin(p); end(p); node=node->next){
		if(node->data>z) x=node;
	}
	return x;
}
...
void show(pList p, bool all, int show_n) {
	...
	for (i = 1, curr=curr; curr != end(p) && i <= show_n; curr = curr->next, i++)
		cout << " > " << curr->data;
	cout << "\n";
}
